'''
https://github.com/AlexanderMelde/SPHAR-Dataset/blob/master/scripts/spatiotemporal_cropping/cutcrop_functions.py
'''
...
def cut_and_crop(mode, input_video_cap, input_fps, input_size, input_video_name, dataset_name,
    ...
    while input_video_cap.isOpened() and frame_returned and (mode != "cv2" or out.isOpened()):
        ...
        if frame_number < timespan[1] and frame is not None:
            x1, y1, x2, y2, f_n, lost, occluded, generated, _ = bbox[int(frame_number) - timespan[0]]

            # exclude occluded and lost frames
            ...
            if crop:
                ...
                frame_cropped = cv2.resize(square_crop, (size_e[0], size_e[1]), interpolation=cv2.INTER_LINEAR)
